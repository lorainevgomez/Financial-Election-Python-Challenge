...
with open(csvpath, encoding='utf') as csvfile:
 ...
 #Loop through profits to find monthly change in profits
    ...
    Total_ProfitLoss_Change_Average= round(Total_ProfitLoss_Change/len(ProfitLoss_Change_List),2)
...
with open(output_file, "w") as datafile:
   ...
   datafile.write('\n')
   datafile.write(f"Average Change: " + "$" + str(Total_ProfitLoss_Change_Average))
   datafile.write('\n')
   datafile.write(f"Greatest Increase in Profits: {Max_Profit_Month} (${Greatest_Inc})")
   datafile.write('\n')
   datafile.write(f"Greatest Decrease in Profits: {Min_Profit_Month} (${Greatest_Dec})")